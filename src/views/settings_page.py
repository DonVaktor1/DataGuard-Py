...
if "saving_settings" not in st.session_state:
    st.session_state.saving_settings = False

if st.session_state.get("settings_updated"):
...
try:
    ...
    with st.form("edit_profile_form"):
        ...
        submit_btn = st.form_submit_button("Зберегти зміни", use_container_width=True, disabled=st.session_state.saving_settings)
        
        if submit_btn:
            if not new_db_name or not new_conn_string:
                st.toast("Усі поля повинні бути заповнені!")
            else:
                st.session_state.saving_settings = True
                st.rerun()

    if st.session_state.saving_settings:
